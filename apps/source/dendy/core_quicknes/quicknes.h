...
/* Returns the size of the internal qnes_state_t struct.
 * Caller must pvPortMalloc this much, zero it, and store
 * the pointer at r9[0] (app_globals_t.qnes_state) before
 * calling any other qnes_* function. */
unsigned long qnes_state_size(void);

/* Set pixel buffer pointers.  Each buffer must be at least
 * (256+16)*(240+2) = 65824 bytes.  Call before qnes_init(). */
